...
int main(void) {
    ...
    // Настройка WatchDog, нужно включать во фьюзах
    asm("wdr");    // Сбрасываем, wdt_reset();
    // Разрешаем ватчдог 2 сек, отключение - wdt_disable()
    WDTCR |= (1<<WDP2) | (1<<WDP1) | (1<<WDP0); // 2 секунды переполнение, wdt_enable(WDTO_2S);
    // Разрешаем прерывания по ватчдогу. Иначе будет резет.
    //WDTCR |= (1<<WDCE); // Флаг сброса микроконтроллера
    WDTCR |= (1<<WDTIE);  // Флаг прерывания по переполнению
